...
census_pop2016 %>% 
  as_tibble() %>% 
  group_by(pr_english) %>% 
  summarise(pop = sum(population_2016), .groups = 'drop') %>% 
  arrange(desc(pop)) %>%
  adorn_totals("row") %>% 
  mutate(perc = round(pop / 35151728 * 100, 1)) %>% 
  mutate(pop = formatC(pop, format="d", big.mark=",")) %>% 
  rename(Province = `pr_english`, Population = pop, 
         `Percent of total` = perc) %>% 
  kbl(caption = "Table 1. Population sizes by province", 
      align = c('l', 'r', 'r')) %>%
...
```
<p class = "fignotes"> <strong>Notes:</strong> Data taken from population sizes from 2016 Data taken from the [MapCan package](https://cran.r-project.org/web/packages/mapcan/vignettes/choropleth_maps_vignette.html). </p>
